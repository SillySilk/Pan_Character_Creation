...
export function PersonalityTable({ tableId, onComplete }: PersonalityTableProps) {
  ...
  const handleRoll = async (rollValue?: number) => {
    ...
    try {
      const rollResult = await tableEngine.processTable(table.id, character, rollValue ? { manualSelection: rollValue } : {})
      ...
      if (rollResult.entry?.effects) {
        for (const effect of rollResult.entry.effects) {
          if (effect.type === 'trait' && effect.target === 'values') {
            ...
            const category = (effect.value?.category || 'neutral') as 'lightside' | 'neutral' | 'darkside' | 'exotic'
            updatePersonalityTraits({
              ...character.personalityTraits,
              [category]: [
                ...(character.personalityTraits?.[category] || []),
                newTrait
              ]
            })
          }
      ...
      {!result && table.entries && (
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
            {table.entries.slice(0, 6).map((entry,_index) => (
